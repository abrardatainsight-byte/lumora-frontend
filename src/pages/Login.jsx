  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,#EEF2FF 0%,#F8FAFC 50%,#FFF7ED 100%)", fontFamily: '"Inter",system-ui,sans-serif' }}>
      <div style={{ width: "100%", maxWidth: 400, padding: "0 20px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, padding: "32px 32px 28px", boxShadow: "0 4px 24px rgba(0,0,0,.08)" }}>
          ...
          <div style={{ display: "flex", background: "#F1F5F9", borderRadius: 10, padding: 3, marginBottom: 24, gap: 3 }}>
            {["employee", "hr"].map((r) => (
              <button key={r} type="button" onClick={() => setRole(r)}
                style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, background: role === r ? "#fff" : "transparent", color: role === r ? "#0F172A" : "#64748B", boxShadow: role === r ? "0 1px 3px rgba(0,0,0,.1)" : "none" }}>
                {r === "hr" ? "HR Admin" : "Employee"}
              </button>
            ))}
          </div>

          {error && <div style={{ padding: "10px 14px", background: "#FEF2F2", color: "#991B1B", borderRadius: 10, marginBottom: 16, fontSize: 13, border: "1px solid #FECACA" }}>{error}</div>}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              <select value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} required
                style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: "1px solid #E2E8F0", fontSize: 14, outline: "none", background: "#fff", cursor: "pointer" }}>
            ...
            <button type="submit" disabled={loading}
              style={{ width: "100%", padding: "13px 0", background: loading ? "#94A3B8" : "linear-gradient(135deg,#4F46E5,#3B82F6)", color: "#fff", border: "none", borderRadius: 10, cursor: loading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 15, marginTop: 4 }}>
